...
let VERSION = "1.0.0";
let levelList = ['log', 'debug', 'info', 'warn', 'error', 'silent'];
let __config = {
...
class log4j {
    ...
    exec(level, msg, tag) {
        ...
        let configLevel = levelList.indexOf(level);
        let outputLevel = levelList.indexOf(this.__config.level);

        if (outputLevel > configLevel)
            return;
        let needLog = false;
        //过滤打印信息
        if (!tag) {
